...
Tensor nms_clsw_impl(Tensor boxes, Tensor scores, std::vector<float> iou_thresholds, int offset) {
  return DISPATCH_DEVICE_IMPL(nms_clsw_impl, boxes, scores, iou_thresholds, offset);
}
...
Tensor nms_clsw(Tensor boxes, Tensor scores, std::vector<float> iou_thresholds, int offset) {
  return nms_clsw_impl(boxes, scores, iou_thresholds, offset);
}
