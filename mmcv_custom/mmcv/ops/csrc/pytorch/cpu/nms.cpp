...
Tensor nms_clsw_cpu(Tensor boxes, Tensor scores, std::vector<float> iou_thresholds, int offset) {
  if (boxes.numel() == 0) {
    return at::empty({0}, boxes.options().dtype(at::kLong));
  }
  auto x1_t = boxes.select(1, 0).contiguous();
  ...
  for (int64_t _i = 0; _i < nboxes; _i++) {
    ...
    for (int64_t _j = _i + 1; _j < nboxes; _j++) {
      ...
      auto inter = w * h;
      auto ovr = inter / (iarea + areas[j] - inter);
      if (ovr > iou_thresholds[i]) select[_j] = false;
    }
  }
  return order_t.masked_select(select_t);
}
...
REGISTER_DEVICE_IMPL(nms_clsw_impl, CPU, nms_clsw_cpu);
